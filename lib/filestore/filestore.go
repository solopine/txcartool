...
func (fs fileStore) MakeLinkFile(rawFilePath string) (File, error) {
	_, rawFile := filepath.Split(rawFilePath)
	linkFilePath := fs.filename(Path(rawFile))
	err := os.Symlink(rawFilePath, linkFilePath)
	if err != nil {
		return nil, err
	}

	return newFile(OsPath(fs.base), Path(rawFile))
}
